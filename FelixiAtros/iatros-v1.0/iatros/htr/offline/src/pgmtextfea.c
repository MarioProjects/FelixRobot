               ...
               float *w, int maxval, char Type)
{
  ...
  float *y=NULL, slope=0.0, aux, aux2;
  int i, j, n=0;
  ...
  aux2= sumA*sumA-sumW*sumAX;
  if ( aux2 != 0.0 )
    {
      slope=(sumB*sumA-sumW*sumBX)/aux2;
      return(atan(slope));
    }
  else return (float) Sing_Value1;
...
*/
 ...
 while ((option=getopt(argc,argv,"shvc:i:o:f:V:O:F:n"))!=-1)
    switch (option)
      {
      case 'n':
	 version(); 
  ...
  if (ifd==NULL ) {
    fprintf(stderr,"El fichero de entrada no puede abrirse o no existe\n");
    exit(-1);
  }
  ...
  if (frec_factor==0) frec_factor=idata.COLS/(asp_rat*udata.cells);   /****/

...
int wait_complete_file(char *fn) {
  struct stat fs;
  int olds=0, news=1;
...
/*
    FILE *samples_file = gzopen(samples_fn, "r");
...
*/
  ...
  REQUIRE(transcriptions == NULL || translations == NULL,
          "Transcriptions and translations are not allowed to be given at the same time");

  FILE *refs_f = NULL;
  if (transcriptions != NULL) {
    refs_f = gzopen(transcriptions, "r");
    CHECK_SYS_ERROR(refs_f != NULL, "Couldn't open file of transcriptions '%s'\n", transcriptions);
    ref_type = REF_SOURCE;
  }
  else if (translations != NULL) {
    refs_f = gzopen(translations, "r");
  ...
  /* Waiting for the sample file to appear
     It will be the file with name in string "samples_fn" */
  char line[MAX_LINE];
  strcpy(line,samples_fn);
  int line_number = 1;
  int size = 0;
  features_t *feas;
  while (1) { /* Infinite loop broken when empty file is the one to be read */
    // do only this part
    /* if (line_number >= start_line && line_number <= end_line) { */

      //Read cepstrals
      printf("Waiting for %s to exist...\n", line);
      while (access(line, F_OK) == -1);                            /* Waits for file existence */
      size = wait_complete_file(line);                             /* Waits for file completion */
      if (size > 1) 
        feas = features_create_from_file(line);                    /* Read features if not empty file */
      remove(line);                                                /* Deletes file for next iteration */
      if (size <= 1) break;                                        /* If empty file finishes process */
      ...
      if (ref_type != REF_NONE && refs_f != NULL) {
        ...
        if (decoder->grammar->start != VOCAB_NONE) {
          ...
          if (ref_type == REF_SOURCE && start->input != NULL) {
        ...
        if (do_forced_recognition) {
          ...
          CALLGRIND_START_INSTRUMENTATION;
          int n_iter = cat_decode(search, feas, lattice, ref, ref_type);
          CALLGRIND_STOP_INSTRUMENTATION;

...
bool grammar_is_end_word(const grammar_t *grammar, symbol_t word) {
  if (grammar->end != VOCAB_NONE) {
    if (word == grammar->end) return true;
    else                      return false;
  }
  else return true;
}
...
/// adds a state in the grammar for the silence symbol
/**
@param grammar Grammar
*/
void grammar_add_silence_state(grammar_t *grammar) {
  //XXX: is the silence state really necessary? no grammar state points to it
  // Besides, the silence is not a language model event nd should not be in the grammar
  if (grammar->silence != VOCAB_NONE) {
    state_grammar_t *silence_state = state_grammar_create();
    grammar_append(grammar, silence_state);
    symbol_t state_name[] = { grammar->silence, VOCAB_NONE };
    state_grammar_set_name(silence_state, state_name);
  }
}
...
//    grammar_load_lat(grammar, file);
  ...
  //XXX: most certainly, the silence state is unnecessary
  //grammar_add_silence_state(grammar);

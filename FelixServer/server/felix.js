...
* sit
**********************************************************/
Felix.prototype.sit = function(){
  //Previamente a todo movimiento se hace -> holdUp()
  //para cualquier movimiento que se este dando este momento
  clearTimeout(this._intervalId); 

  var theSitPose = [];

  theSitPose.push({cmd:'pose',angles:[{hip:70,knee:140},{hip:70,knee:140},{hip:40,knee:83},{hip:40,knee:83}]}); 
  theSitPose.push({cmd:'pose',angles:[{hip:70,knee:140},{hip:70,knee:140},{hip:48,knee:83},{hip:48,knee:83}]}); 

  //asigna tu secuencia
  this.schedule = theSitPose;
  this.cycle_idx = 0;
  this.scheduleIdx = 0;

  //empieza a executar tu secuencia
  this._runSchedule();
}


/**********************************************************
* hold up -> Move all to homeLeg position
**********************************************************/
Felix.prototype.holdUp = function(){
  clearTimeout(this._intervalId); 
  this.homeLeg("FR");
  this.homeLeg("FL");
  this.homeLeg("BR");
...
Felix.prototype.paw = function(){
  ...
  this.sit();
  //Supuestamente -> FR,FL,BR,BL
  thePaw.push({cmd:'pose',angles:[{hip:144,knee:150},{hip:70,knee:140},{hip:48,knee:85},{hip:48,knee:73}]});

  //asigna tu secuencia
  this.schedule = thePaw;
  this.cycle_idx = 0;
  this.scheduleIdx = 0;
...
* get down
**********************************************************/
Felix.prototype.getDown = function(){
  //Previamente a todo movimiento se hace -> holdUp()
  //para cualquier movimiento que se este dando este momento
  clearTimeout(this._intervalId); 
  ...
  theGetDown.push({cmd:'pose',angles:[{hip:40,knee:83},{hip:40,knee:83},{hip:70,knee:140},{hip:70,knee:140}]}); 
  theGetDown.push({cmd:'pose',angles:[{hip:48,knee:83},{hip:48,knee:83},{hip:70,knee:140},{hip:70,knee:140}]}); 

  //asigna tu secuencia
  this.schedule = theGetDown;
  this.cycle_idx = 0;
  this.scheduleIdx = 0;
...
/**********************************************************
* method to make felix put determinated Pose
* @param thePose array with the pose that felix will put
**********************************************************/
Felix.prototype.setPose = function(thePose){
  ...
  //primera pose (FR,FL,BR,BL)
  //secuencia.push({cmd:'pose',angles:[{hip:90,knee:90},{hip:90,knee:90},{hip:90,knee:90},{hip:90,knee:90}]}); //aqui tienes que definir los angulor correctos
...
Felix.prototype._runSchedule = function(){
  console.log('runSchedule:'+ this.scheduleIdx);
  if(this.scheduleIdx  >= this.schedule.length) return;

  var frame = this.schedule[this.scheduleIdx];

  if(frame.cmd == 'pause'){
    if(this.framePause == -1) this.framePause = frame.count;
...
Felix.prototype._legSetup = function(){
  ...
  for(var i = 0; i < this.legs.length; i++){
    this.legs[i].hip.servo = new this.J5.Servo({
                          ...
                          range:[0,180],
                          specs: { speed:this.J5.Servo.Continuous.speeds["@3.3V"] }
                        });
